...
import pickle
#from TfUtils import mkMask #entry_stop_gradients, mkMask, reduce_avg, masked_softmax
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if ex_index < 4:
        ...
        tf.logging.info("text_a %s" % (example.text_a))  #  add
...
def routing_masked(input_tensor, seq_length, hidden_size, out_caps_num=5, iter=3,
                  scope=None):
    batch_size = tf.shape(input_tensor)[0]
    with tf.variable_scope(scope or 'routing'):
        caps = Capsule(hidden_size, out_caps_num, iter)(input_tensor, seq_length)
        flatted_caps = tf.reshape(caps,shape=[batch_size,out_caps_num*hidden_size])
        print("Capsule has finshed！")
        return flatted_caps
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    # 128 seq length
    # 32 batch size
    output_layer = model.get_sequence_output() #model.get_pooled_output()
    seq_length, hidden_size = output_layer.shape[-2].value, output_layer.shape[-1].value
    sent_rep = routing_masked(output_layer, seq_length,hidden_size, FLAGS.caps_iter , 3, scope='attention')
    sent_rep_last_dim = int(sent_rep.shape[-1])

    output_weights = tf.get_variable(
        "output_weights", [num_labels, sent_rep_last_dim],
    ...
    with tf.variable_scope("loss"):
        if is_training:
            # I.e., 0.1 dropout
            sent_rep = tf.nn.dropout(sent_rep, keep_prob=0.9)

        logits = tf.matmul(sent_rep, output_weights, transpose_b=True)
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits):
                ...
                tf.summary.scalar('eval_recall', recall[1]) # add
